...
class ButtonToolbarItem(AbstractControl):
    """
    按钮组
    """
...
class ChainSelectItem(AbstractControl):
    # 链式下拉框
    pass


class CheckboxItem(AbstractControl):
    # 勾选框
    type = "checkbox"
...
class CheckboxesItem(AbstractControl):
    # 复选框
    type = "checkboxes"
...
class CityItem(AbstractControl):
    # 城市选择器
    pass


class ColorItem(AbstractControl):
    # 颜色选择器
    pass


class ComboControlItem(AbstractControl):
    # Combo组合中的表单项
    type = "combo-control"
...
class ComboItem(AbstractControl):
    # Combo组合
    type = "form"
    debug: bool = True
...
class EditorItem(AbstractControl):
    # 代码编辑器
    pass


class FieldSetItem(AbstractControl):
    # 表单项集合
    type = "form"
    api: Optional[HttpUrl]
...
class FileItem(AbstractControl):
    # 文件上传
    type = "form"
...
class FormulaItem(AbstractControl):
    # Formula公式
    pass


class GroupItem(AbstractControl):
    # Group表单项组
    pass


class HBoxItem(AbstractControl):
    # HBox
    pass


class HiddenItem(AbstractControl):
    # Hidden隐藏字段
    pass


class ImageItem(AbstractControl):
    # Image图片
    type = "image"
    reciever: Optional[HttpUrl]  # 上传文件接口
...
class InputGroupItem(AbstractControl):
    # 输入框组合
    pass


class ListSchemaItem(AbstractControl):
...
class UuidItem(AbstractControl):
    # 随机生成一个 id，可以用于防止表单重复提交。
    type = ControlEnum.uuid
    name: Optional[str] = "uuid"
    length: Optional[int]


class MatrixItem(AbstractControl):
    # 矩阵类型的输入框。
    type = "page"


class NestedSelectItem(AbstractControl):
    # NestedSelect 级联选择器
    type = "page"
    body: Optional[dict]


class PanelItem(AbstractControl):
    # 展现上将多个 表单项 放同一个容器下。
    type = "page"


class PickerItem(AbstractControl):
    # 列表选取，在功能上和 Select 类似，但它能显示更复杂的信息。默认和 Select 很像，但请看后面的 pickerSchema 设置。
    type = "page"


class RadiosItem(AbstractControl):
    # 用于实现单选。
    type = "page"
...
class RatingItem(AbstractControl):
    # Rating 评分
    type = "page"


class RangeItem(AbstractControl):
    # Range 滑块,可以用于选择单个数值或数值范围。
    type = "page"
    body: Optional[Dict[str, Any]]


class RepeatItem(AbstractControl):
    # Repeat 重复频率选择器
    type = "page"
    body: Optional[Dict[str, Any]]


class RichTextItem(AbstractControl):
    # 目前富文本编辑器基于两个库：froala 和 tinymce，默认使用 tinymce。
    type = "page"
    body: Optional[Dict[str, Any]]


class ServiceItem(AbstractControl):
    # Service 功能容器
    type = "page"
    body: Optional[Dict[str, Any]]


class SubFormItem(AbstractControl):
    # SubForm 子表单
    type = "page"
    body: Optional[Dict[str, Any]]


class SwitchItem(AbstractControl):
    # Switch 开关
...
class StaticItem(AbstractControl):
    # Static 静态展示   用来在表单中，展示静态数据
    type = "page"
    body: Optional[Dict[str, Any]]


class TabsItem(AbstractControl):
    # Tabs 选项卡   有多组输入框时，也可以通过选项卡来分组。
    type = "page"
    body: Optional[Dict[str, Any]]


class TableItem(AbstractControl):
    # Table 表格
    type = "table"
    body: Optional[Dict[str, Any]]
...
class TextItem(AbstractControl):
    # Text 输入框
...
class TimeItem(AbstractControl):
    # Time 时间
    type = "page"
    body: Optional[Dict[str, Any]]
...
class TransferItem(AbstractControl):
    # Transfer 穿梭器
    type = "page"
    options: List[Union[dict, str]]  # 选项组
    source: Union[str, HttpUrl]  # 动态选项组
...
class TabsTransferItem(AbstractControl):
    # TabsTransfer 组合穿梭器
    pass


class TreeItem(AbstractControl):
    # Tree 树形选择框
    type = "page"
    body: Optional[Dict[str, Any]]
    options: List[Union[dict, str]]  # 选项组
...
class YearItem(AbstractControl):
    # Year年
    type = "page"
    body: Optional[Dict[str, Any]]
