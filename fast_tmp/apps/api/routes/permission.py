...
from fast_tmp.apps.api.schemas import (
...
tpl.add_create_button(
    "post:/permission", get_controls_from_model(Permission, exclude=("id", "groups"))
)
tpl.add_modify_button(
    get_api="get:/permission/?id=${id}",
    put_api="put:/permission/${id}",
...
async def post_permissions(
    permission_schema: permission_create_schema, user: User = Depends(get_superuser)
):
...
async def put_permission(
    permission_schema: permission_create_schema, id: int, user: User = Depends(get_superuser)
):
