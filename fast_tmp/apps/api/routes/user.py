...
from fast_tmp.apps.api.schemas import user_create_schema, user_list_schema, user_schema
from fast_tmp.depends import PageDepend, get_superuser, page_depend
...
tpl.add_create_button("post:/post", get_controls_from_model(User, exclude=("id", "groups")))
tpl.add_modify_button(
    get_api="get:/get/?id=${id}",
    put_api="put:/put/${id}",
...
async def post_users(user_info: user_create_schema, user: User = Depends(get_superuser)):
...
async def put_user(userinfo: user_create_schema, id: int, user: User = Depends(get_superuser)):
